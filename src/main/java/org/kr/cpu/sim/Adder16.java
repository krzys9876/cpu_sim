...
public class Adder16 extends Component {
    ...
    private static InputPin[] initInputPins(String prefix, int startPin, int count) {
        InputPin[] arr = new InputPin[count];
        for(int i = 0; i < arr.length; i++) arr[i] = new InputPin(prefix + i, startPin + i);
        return arr;
    }

    private static OutputPin[] initOutputPins(String prefix, int startPin, int count) {
    ...
    public Component setInput(int pinNo, boolean value) {
        ...
        else if (pinNo <=8) { state[0].setInput(pinNo, value); setInternalCarry(); } // ADD1 A
        else if (pinNo <=16) { state[1].setInput(pinNo-8, value); } // ADD2 A
        else if (pinNo <=24) { state[0].setInput(pinNo-8, value); setInternalCarry(); } // ADD1 B
        else  { state[1].setInput(pinNo-16, value); } // ADD2 B
