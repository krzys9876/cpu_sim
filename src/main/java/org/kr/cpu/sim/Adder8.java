...
public class Adder8 extends Component {
    ...
    public void setInput(int[] pinNo, boolean[] value) {
        ...
        for (int i = 0; i < pinNo.length; i++) { //super.setInput(pinNo[i], value[i]);
            if (pinNo[i] == 0) { state[0].setInput(Adder4.PIN_C0.order, value[i]); setInternalCarry(); } // ADD1 C0
            else if (pinNo[i] <=4) { state[0].setInput(Adder4.PIN_A[pinNo[i]-1].order, value[i]); setInternalCarry(); } // ADD1 A
            else if (pinNo[i] <=8) { state[1].setInput(Adder4.PIN_A[pinNo[i]-5].order, value[i]); } // ADD2 A
            else if (pinNo[i] <=12) { state[0].setInput(Adder4.PIN_B[pinNo[i]-9].order, value[i]); setInternalCarry(); } // ADD1 B
            else  { state[1].setInput(Adder4.PIN_B[pinNo[i]-13].order, value[i]); } // ADD2 B
        }
        updateOutput();
    }
}
