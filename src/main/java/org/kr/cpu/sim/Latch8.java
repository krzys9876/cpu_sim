...
public class Latch8 extends Component {
    ...
    public Latch8(String id) {
        ...
        for(int i=0; i<8; i++) setOutput(PIN_Q[i].order, false);
    }

    private final boolean[] state = new boolean[] {false, false, false, false, false, false, false, false};
    @Override
    ...
    public void setInput(int pinNo, boolean value, boolean shouldRefresh) {
        ...
        if(clkRisingEdge) {
            // store state due to rising clock edge (NOTE: clock should be updated AFTER all data lines)
